...
namespace Rujta.Infrastructure.Repositories
{
    public class PaymentRepository : GenericRepository<Payment, int>, IPaymentRepository
    {
        ...
        // PaymobTransactionId بقت nullable — نتجنب match على null
        public async Task<Payment?> GetByTransactionIdAsync(
            string transactionId,
            CancellationToken cancellationToken = default)
            => await _context.Payments
                .FirstOrDefaultAsync(
                    p => p.PaymobTransactionId != null &&
        ...
        public async Task<Payment?> GetByOrderIdAsync(
            ...
            CancellationToken cancellationToken = default)
            => await _context.Payments
                .Where(p => p.OrderId == orderId && p.Type == PaymentType.Order)
                .OrderByDescending(p => p.CreatedAt)
                .FirstOrDefaultAsync(cancellationToken);

        public async Task<IEnumerable<Payment>> GetByPharmacyIdAsync(
            int pharmacyId,
            CancellationToken cancellationToken = default)
            => await _context.Payments
        ...
        public async Task<IEnumerable<Payment>> GetByPharmacyAndTypeAsync(
            int pharmacyId,
            PaymentType type,
            CancellationToken cancellationToken = default)
            => await _context.Payments
