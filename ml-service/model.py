...
model.py
--------
Graph utilities and MR-GNN model definition for Drug-Drug Interaction prediction.
(Xu et al., IJCAI 2019 — faithful implementation)
"""
...
def batch_graphs(graph_list):
    """Collate a list of GraphData into a single GraphBatch."""
...
def _clean_smiles_cached(smiles_str: str):
    ...
    try:
        ...
        if mol is None:
            return None
        try:
            mol = remover.StripMol(mol, dontUseReactions=True, dontRemoveEverything=True)
        except Exception:
            pass
        try:
...
class WeightedGCL(nn.Module):
    """Weighted Graph Convolution Layer (paper Eq. 2)."""

    def __init__(self, in_dim, out_dim, max_degree=10):
        super().__init__()
        self.max_degree = max_degree
        self.phi  = nn.ModuleList([nn.Linear(in_dim, out_dim, bias=False)
                                   for _ in range(max_degree + 1)])
        self.psi  = nn.ModuleList([nn.Linear(in_dim, out_dim, bias=False)
                                   for _ in range(max_degree + 1)])
        self.bias = nn.ParameterList([nn.Parameter(torch.zeros(out_dim))
                                      for _ in range(max_degree + 1)])
...
class GraphPooling(nn.Module):
    """Neighbourhood max pooling (paper Eq. 3)."""
...
class GraphGather(nn.Module):
    """Graph-Gather readout (paper Eq. 4)."""

    def __init__(self, in_dim, graph_dim, max_degree=10):
        super().__init__()
        self.max_degree = max_degree
        self.theta = nn.ModuleList([nn.Linear(in_dim, graph_dim, bias=False)
                                    for _ in range(max_degree + 1)])
        self.beta  = nn.ParameterList([nn.Parameter(torch.zeros(graph_dim))
                                       for _ in range(max_degree + 1)])
...
class MRGNN(nn.Module):
    """
    Faithful implementation of MR-GNN (Xu et al., IJCAI 2019).

    Default hyper-parameters match the paper (binary DDI task):
        conv_dim   = 384
        graph_dim  = 128
        hidden_dim = 512
        num_layers = 3
