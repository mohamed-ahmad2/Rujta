...
export const useCheckout = () => {
  ...
  const totalSelectedItems = useMemo(
    () =>
      Object.values(selectedMedicines).reduce(
        (sum, meds) => sum + Object.keys(meds).length, 0,
      ),
    [selectedMedicines],
  );
  ...
  const handleToggleMedicine = useCallback((pharmacyId, medicine) => {
    setSelectedMedicines((prev) => {
      ...
      return { ...prev, [pharmacyId]: { ...current, [medicine.medicineId]: getAvailableQty(medicine) } };
    });
  }, []);

  const handleUpdateQty = useCallback((pharmacyId, medicineId, newQty, maxQty) => {
  ...
  const handleTogglePharmacy = useCallback((pharmacyId, allMedicines = []) => {
    setSelectedMedicines((prev) => {
      const current     = prev[pharmacyId] ?? {};
      const allSelected = allMedicines.length > 0 && allMedicines.every((m) => m.medicineId in current);
      if (allSelected) return { ...prev, [pharmacyId]: {} };
      const newMeds = { ...current };
      allMedicines.forEach((m) => { if (!(m.medicineId in current)) newMeds[m.medicineId] = getAvailableQty(m); });
      ...
      fetch(url)
        ...
        .then((data) => {
          if (data.code === "Ok" && data.routes?.[0]) {
            ...
            setRouteData((p) => ({ ...p, [cacheKey]: { coordinates, distanceKm, durationMin } }));
          }
        })
  ...
  const handleSetLocation = () => {
    navigator.geolocation?.getCurrentPosition(async ({ coords }) => {
      try {
        await apiClient.put("/users/location", { latitude: coords.latitude, longitude: coords.longitude });
  ...
  const handleConfirmAddress = async () => {
    if (!selectedAddressId) { showToast("error", "Please select a delivery address!"); return; }
    setIsConfirmingAddress(true);
    try {
      if (cart.length > 0) await fetchPharmacies(cart, selectedAddressId, pharmaciesRange);
    ...
    finally { setIsConfirmingAddress(false); }
  };
  ...
  const handleOrderClick = (pharmacy) => {
    ...
    setSelectedMedicines({ [pharmacy.pharmacyId]: medsMap });
  ...
  const createOrders = async () => {
    ...
    const orderDtos = selectedPharmacies.reduce((acc, pharmacyId) => {
      const pharmacy = pharmacies.find((p) => String(p.pharmacyId) === String(pharmacyId));
      if (!pharmacy) return acc;
      const selectedMedsMap   = selectedMedicines[pharmacyId] ?? {};
      const normalizedMedsMap = Object.fromEntries(Object.entries(selectedMedsMap).map(([k, v]) => [String(k), v]));
      const items = pharmacy.foundMedicines.filter((m) => String(m.medicineId) in normalizedMedsMap);
      if (!items.length) return acc;
      acc.push({
        PharmacyID:        pharmacy.pharmacyId,
        DeliveryAddressId: selectedAddressId,
        OrderItems: items.map((m) => ({ MedicineID: m.medicineId, Quantity: normalizedMedsMap[String(m.medicineId)] })),
      });
      return acc;
    }, []);

    if (!orderDtos.length) throw new Error("No valid orders. Check selected medicines & quantities.");
  ...
  // ── Return ───────────────────────────────────────────────────────
  return {
    cart, pharmacies, loading, error,
    addresses, addressesLoading, addressesError,
    pharmaciesRange, showLocationPrompt, showAddressSelection,
    selectedAddressId, showNewAddressForm, isConfirmingAddress, newAddressForm,
    expandedPharmacies, showPaymentModal, selectedPharmacyForPayment,
    paymentMethod, selectedPharmacies, totalSelectedItems, creatingOrder,
    selectedMedicines, totalSelectedQtyPerMedicine,
    initiatingPayment, showPaymentIframe, paymentResult,
    userLocation, deliveryAddressLocation, deliveryAddress, hoveredPharmacyId, routeData,
    toast,
    setSelectedAddressId, setShowNewAddressForm, setNewAddressForm,
    setExpandedPharmacies, setShowPaymentModal, setPaymentMethod,
    setHoveredPharmacyId, setToast, setShowAddressSelection,
    handleSetLocation, handleNewAddressChange, handleAddNewAddress,
