...
function AdCard({ ad }) {
  ...
  // ✅ Fixed: explicit status resolution, no operator precedence bug
  const rawStatus = ad.status ?? ad.Status;
  const adStatus = rawStatus
...
export default function Payments() {
  // ── Auth: resolve this pharmacy's ID ──
  const { user } = useAuth();
  ...
  // ✅ fetchByPharmacy(pharmacyId) — scopes ads to THIS pharmacy only
  const { ads, loading: adsLoading, error: adsError, fetchByPharmacy } = useCampaigns();

  const refetchAll = () => {
    allPayments.fetchMyPayments();
    fetchStatus();
    if (pharmacyId) {
      fetchByPharmacy(pharmacyId); // ← only ads belonging to this pharmacy
    }
  };

  // ✅ Wait for pharmacyId before fetching — re-runs once auth resolves
  useEffect(() => {
  ...
  // ✅ Fixed operator precedence bug from original
  const activeAds = (ads ?? []).filter((a) => {
    const status = String(a.status ?? a.Status ?? "").toLowerCase();
    return status === "active" || a.isActive === true;
  });
    ...
    <div className="space-y-5 p-3 sm:p-4 md:p-6 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-2">
        ...
        <div className="space-y-3">
          ...
          {/* ✅ Show helpful state while pharmacyId is still resolving */}
          {!pharmacyId ? (
            <div className="flex items-center gap-2 text-sm text-gray-400 px-1">
              <Loader2 size={13} className="animate-spin" /> Loading account info…
            </div>
          ) : (ads ?? []).length === 0 ? (
