...
function Toast({ message, type, onClose }) {
  ...
  return (
    <div style={{
      position: 'fixed', bottom: '24px', right: '16px', left: '16px', zIndex: 9999,
      ...
      <span style={{ fontSize:'13px', color:colors.text, fontWeight:500, flex:1 }}>{message}</span>
      <button onClick={onClose} style={{ background:'none', border:'none', cursor:'pointer', color:colors.text, fontSize:'16px', lineHeight:1, opacity:0.6 }}>×</button>
    </div>
  )
}
...
function NotificationsPanel({ notifications, onClose }) {
  return (
    <div style={{
      ...
      width: '280px', boxShadow: '0 8px 32px rgba(0,0,0,0.12)', overflow: 'hidden',
    }}>
...
// ─── Mobile Drug Card ─────────────────────────────────────────────────────────
function DrugCard({ drug, isSelected, onClick }) {
  return (
    <div
      onClick={onClick}
      style={{
        background: isSelected ? '#f0f8e8' : '#fff',
        border: isSelected ? '1.5px solid #4caf50' : '0.5px solid #e0e0d8',
        borderRadius: '10px',
        padding: '14px',
        cursor: 'pointer',
    ...
    >
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:'8px' }}>
        <div>
          <div style={{ fontSize:'14px', fontWeight:600, color:'#1a1a1a' }}>{drug.name}</div>
          <div style={{ fontSize:'10px', color:'#bbb', marginTop:'2px' }}>{drug.sku}</div>
        </div>
        <span style={{ padding:'3px 8px', borderRadius:'4px', fontSize:'10px', fontWeight:600, letterSpacing:'0.5px', ...badgeStyles[drug.categoryClass], flexShrink:0 }}>{drug.category}</span>
      </div>
      <div style={{ display:'flex', justifyContent:'space-between', fontSize:'12px', color:'#555' }}>
        <span>{drug.mfr}</span>
        <span style={{ color:'#888' }}>{drug.submittedBy}</span>
      </div>
    </div>
  )
}
...
function DetailDrawer({ d, onClose, onApprove, approving }) {
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display:'flex', justifyContent:'center', padding:'12px 0 4px' }}>
          <div style={{ width:'36px', height:'4px', borderRadius:'2px', background:'#e0e0d8' }} />
        </div>

        {/* Header */}
        <div style={{ padding:'8px 16px 12px', borderBottom:'0.5px solid #e0e0d8', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
          <div style={{ fontSize:'15px', fontWeight:600, color:'#1a1a1a' }}>Request Details</div>
          <button onClick={onClose} style={{ background:'none', border:'none', cursor:'pointer', color:'#aaa', fontSize:'20px' }}>×</button>
        </div>

        {/* Body */}
        <div style={{ overflowY:'auto', flex:1, padding:'16px' }}>
          {/* Pill visual */}
          <div style={{ borderRadius:'10px', overflow:'hidden', height:'110px', background:'linear-gradient(135deg,#d4c4b0,#b8a090)', marginBottom:'16px', position:'relative' }}>
            <div style={{ width:'100%', height:'100%', display:'flex', alignItems:'center', justifyContent:'center' }}>
              <div style={{ display:'grid', gridTemplateColumns:'repeat(6,1fr)', gap:'5px', padding:'12px' }}>
                {Array(12).fill(0).map((_,i) => (
                  <div key={i} style={{ width:'24px', height:'24px', borderRadius:'50%', background:d.pillColor, boxShadow:'inset -2px -2px 5px rgba(0,0,0,0.2)' }}/>
                ))}
              </div>
            </div>
          </div>

          <div style={{ fontSize:'18px', fontWeight:700, color:'#1a1a1a', marginBottom:'12px' }}>{d.name}</div>

          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'12px', marginBottom:'14px' }}>
            {[['Manufacturer', d.mfr], ['Strength', d.strength], ['Submitted by', d.submittedBy], ['Category', d.category]].map(([k,v]) => (
              <div key={k}>
                <div style={{ fontSize:'10px', color:'#aaa', marginBottom:'2px' }}>{k}</div>
                <div style={{ fontSize:'13px', fontWeight:500, color:'#1a1a1a' }}>{v}</div>
              </div>
            ))}
          </div>

          <div style={{ marginBottom:'14px' }}>
            <div style={{ fontSize:'10px', color:'#aaa', marginBottom:'4px' }}>Storage Requirements</div>
            <div style={{ fontSize:'13px', fontWeight:500, color:d.storageColor }}>✳ {d.storage}</div>
          </div>

          <div style={{ background:'#f8fbf4', borderRadius:'8px', padding:'12px' }}>
            <div style={{ fontSize:'10px', color:'#aaa', marginBottom:'6px' }}>Pharmacist Notes</div>
            <div style={{ fontSize:'12px', color:'#555', lineHeight:1.6, fontStyle:'italic', marginBottom:'10px' }}>"{d.notes}"</div>
        ...
        <div style={{ padding:'12px 16px', borderTop:'0.5px solid #e0e0d8' }}>
          <button
            onClick={onApprove}
            disabled={approving}
            style={{ width:'100%', padding:'14px', background: approving ? '#a5d6a7' : '#4caf50', border:'none', borderRadius:'10px', color:'#fff', fontSize:'15px', fontWeight:600, cursor: approving ? 'default' : 'pointer', display:'flex', alignItems:'center', justifyContent:'center', gap:'8px' }}
          >
            {approving ? (
              <>
...
export default function ApprovalQueue() {
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", minHeight:"620px", background:"#f5f5f0", borderRadius:"12px", overflow:"hidden", border:"0.5px solid #e0e0d8", fontFamily:"-apple-system,'Helvetica Neue',sans-serif", position:'relative' }}>

      {/* ── Topbar ── */}
      <div style={{ background:"#fff", borderBottom:"0.5px solid #e0e0d8", padding:"10px 16px", display:"flex", alignItems:"center", gap:"8px", flexWrap:"wrap" }}>
        {/* Search */}
        <div style={{ flex:1, minWidth:'140px', background:"#f5f5f0", border:"0.5px solid #e0e0d8", borderRadius:"8px", padding:"7px 12px", fontSize:"12px", display:"flex", alignItems:"center", gap:"8px" }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:"6px", position:'relative' }}>
          ...
          <div style={{ position:'relative' }}>
            <button
              ...
              style={{ width:"30px", height:"30px", borderRadius:"6px", border:"0.5px solid #e0e0d8", display:"flex", alignItems:"center", justifyContent:"center", background:"#fff", cursor:'pointer', position:'relative' }}
            >
          ...
          <button
            ...
            style={{ padding:"5px 10px", border:"1.5px solid #e53935", borderRadius:"6px", fontSize:"11px", color:"#e53935", fontWeight:500, background:"#fff", cursor:'pointer', whiteSpace:'nowrap' }}
          >
            Emergency
          </button>
        </div>
      ...
      <div style={{ display:"flex", flex:1, overflow:"hidden" }}>
        ...
        <div style={{ flex:1, padding:"16px", overflowY:"auto" }}>
          <div style={{ fontSize:"20px", fontWeight:700, color:"#1a1a1a", marginBottom:"4px" }}>Approval Queue</div>
          <div style={{ fontSize:"12px", color:"#888", marginBottom:"16px", lineHeight:1.5 }}>
            Review and approve new medication requests.
            {searchQ && <span style={{ color:'#4caf50', marginLeft:'4px' }}>Results for "{searchQ}"</span>}
          </div>

          {/* ── MOBILE: Cards ── */}
          <div className="sm:hidden" style={{ display:'flex', flexDirection:'column', gap:'10px' }}>
            {pageData.length === 0 ? (
              <div style={{ padding:'32px', textAlign:'center', color:'#bbb', fontSize:'13px', background:'#fff', borderRadius:'10px', border:'0.5px solid #e0e0d8' }}>
                No results found{searchQ ? ` for "${searchQ}"` : ''}
              </div>
            ) : pageData.map((drug) => (
              <DrugCard
                key={drug.sku}
                drug={drug}
                isSelected={selected === drug.sku}
                onClick={() => setSelected(drug.sku)}
              />
            ))}
          </div>

          {/* ── DESKTOP: Table ── */}
          <div style={{ background:"#fff", borderRadius:"10px", border:"0.5px solid #e0e0d8", overflow:"hidden", display:'none' }} className="hidden sm:block">
            ...
            {pageData.length === 0 ? (
              ...
              <div key={drug.sku} onClick={() => setSelected(drug.sku)}
                style={{
          ...
          <div style={{ marginTop:'14px', display:"flex", alignItems:"center", justifyContent:"space-between" }}>
            <div style={{ fontSize:"11px", color:"#aaa" }}>
              {pageData.length} of {filtered.length} {searchQ ? 'results' : 'pending'}
            </div>
            <div style={{ display:"flex", gap:"4px", alignItems:'center' }}>
              <button onClick={() => setPage(p => Math.max(1, p-1))} disabled={page === 1}
                style={{ width:"28px", height:"28px", border:"0.5px solid #e0e0d8", borderRadius:"6px", background:"#fff", cursor: page===1?"default":"pointer", fontSize:"14px", color: page===1?"#ddd":"#666", opacity: page===1?0.5:1 }}>‹</button>
              {Array.from({ length: totalPages }, (_, i) => (
                <button key={i} onClick={() => setPage(i+1)}
                  style={{ width:"28px", height:"28px", border:"0.5px solid #e0e0d8", borderRadius:"6px", background: page===i+1?"#4caf50":"#fff", color: page===i+1?"#fff":"#666", fontSize:"12px", cursor:"pointer", fontWeight: page===i+1?600:400 }}>{i+1}</button>
              ))}
              <button onClick={() => setPage(p => Math.min(totalPages, p+1))} disabled={page === totalPages}
                style={{ width:"28px", height:"28px", border:"0.5px solid #e0e0d8", borderRadius:"6px", background:"#fff", cursor: page===totalPages?"default":"pointer", fontSize:"14px", color: page===totalPages?"#ddd":"#666", opacity: page===totalPages?0.5:1 }}>›</button>
            </div>
          </div>
        </div>
        ...
        {d && (
          <div className="hidden sm:flex" style={{ width:"320px", background:"#fff", borderLeft:"0.5px solid #e0e0d8", flexDirection:"column", overflow:"hidden" }}>
            <div style={{ padding:"14px 16px", borderBottom:"0.5px solid #e0e0d8", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
              ...
              <button onClick={() => setSelected(null)} style={{ width:"22px", height:"22px", border:"none", background:"none", cursor:"pointer", color:"#aaa", fontSize:"18px" }}>×</button>
            </div>
            ...
            <div style={{ flex:1, overflowY:"auto", padding:"14px 16px" }}>
              <div style={{ fontSize:"10px", color:"#aaa", marginBottom:"3px" }}>Drug Name</div>
              <div style={{ fontSize:"18px", fontWeight:700, color:"#1a1a1a", marginBottom:"14px" }}>{d.name}</div>
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"12px", marginBottom:"12px" }}>
                <div><div style={{ fontSize:"10px", color:"#aaa" }}>Manufacturer</div><div style={{ fontSize:"13px", fontWeight:500, color:"#1a1a1a", marginTop:"2px" }}>{d.mfr}</div></div>
                <div><div style={{ fontSize:"10px", color:"#aaa" }}>Strength</div><div style={{ fontSize:"13px", fontWeight:500, color:"#1a1a1a", marginTop:"2px" }}>{d.strength}</div></div>
              </div>
              <div style={{ marginBottom:"14px" }}>
                <div style={{ fontSize:"10px", color:"#aaa" }}>Storage Requirements</div>
                <div style={{ fontSize:"13px", fontWeight:500, color:d.storageColor, marginTop:"3px" }}>✳ {d.storage}</div>
              </div>
              <div style={{ background:"#f8fbf4", borderRadius:"8px", padding:"12px" }}>
                <div style={{ fontSize:"10px", color:"#aaa", marginBottom:"8px" }}>Pharmacist Notes</div>
            ...
            <div style={{ padding:"12px 16px", borderTop:"0.5px solid #e0e0d8" }}>
              <button
                ...
                disabled={approving}
                style={{ width:"100%", padding:"12px", background: approving ? "#a5d6a7" : "#4caf50", border:"none", borderRadius:"8px", color:"#fff", fontSize:"14px", fontWeight:600, cursor: approving ? "default" : "pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:"8px" }}
              >
                {approving ? (<><div style={{ width:'16px', height:'16px', border:'2px solid rgba(255,255,255,0.4)', borderTop:'2px solid #fff', borderRadius:'50%', animation:'spin 0.7s linear infinite' }}/>Approving...</>) : '✓ Approve & Add to Database'}
              </button>
              <style>{`@keyframes spin { to { transform: rotate(360deg) } }`}</style>
            </div>
          </div>
      ...
      {d && (
        <div className="sm:hidden">
          <DetailDrawer d={d} onClose={() => setSelected(null)} onApprove={handleApprove} approving={approving} />
        </div>
      )}
      ...
      {showEmergency && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.4)', display:'flex', alignItems:'flex-end', justifyContent:'center', zIndex:200 }}>
          <div style={{ background:'#fff', borderRadius:'20px 20px 0 0', padding:'24px', width:'100%', maxWidth:'480px', boxShadow:'0 8px 40px rgba(0,0,0,0.2)' }}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'16px' }}>
              <div>
                <div style={{ fontSize:'16px', fontWeight:700, color:'#c62828' }}>Emergency Override</div>
                <div style={{ fontSize:'11px', color:'#aaa', marginTop:'2px' }}>This will be logged and reviewed</div>
              </div>
            ...
            <textarea
              ...
              style={{ width:'100%', border:'1px solid #e0e0d8', borderRadius:'8px', padding:'10px', fontSize:'13px', height:'90px', resize:'none', outline:'none', fontFamily:'inherit', boxSizing:'border-box' }}
            />
            <div style={{ display:'flex', gap:'8px', marginTop:'12px' }}>
              <button onClick={() => setShowEmergency(false)} style={{ flex:1, padding:'12px', border:'1px solid #e0e0d8', borderRadius:'8px', fontSize:'13px', cursor:'pointer', background:'#fff', color:'#555' }}>Cancel</button>
              <button
                onClick={handleEmergencySubmit}
                disabled={!emergencyText.trim()}
                style={{ flex:1, padding:'12px', background: emergencyText.trim() ? '#e53935':'#ffcdd2', border:'none', borderRadius:'8px', fontSize:'13px', color:'#fff', fontWeight:600, cursor: emergencyText.trim() ? 'pointer':'default' }}
