    ...
    <div style={{
      ...
      {notifications.length === 0 ? (
        ...
        notifications.map((n, i) => (
          <div key={i} style={{
            padding:'12px 16px',
            borderBottom: i < notifications.length - 1 ? '0.5px solid #f0f0e8' : 'none',
...
export default function ApprovalQueue() {
  ...
  const { connection } = useAdminNotifications()

  const [refreshKey, setRefreshKey]           = useState(0)   // ✅ forces fetchAll to re-run
  ...
  // ✅ re-runs whenever refreshKey bumps
  useEffect(() => { fetchAll('Pending') }, [fetchAll, refreshKey])
  ...
  // ================= New drug request arrives (SuperAdmin sees it instantly) =================
  useEffect(() => {
    if (!connection) return
    const handleNewDrugRequest = (data) => {
      triggerRefresh()   // ✅ bumps key → fetchAll re-runs → table updates
      showToast(`💊 New request: "${data.drugName}" from Pharmacy #${data.pharmacyId}`, "success")
      addNotification(`New drug request: "${data.drugName}" from Pharmacy #${data.pharmacyId}`, "success")
    }
  ...
  // ================= Request reviewed — remove from queue instantly =================
  useEffect(() => {
    if (!connection) return
    const handleReviewed = (data) => {
      console.log("✅ Review complete, refreshing queue:", data)
      triggerRefresh()   // ✅ re-fetches pending list, reviewed item is gone
      setSelected(null)
    }
      ...
      <div style={{ background:"#fff", borderBottom:"0.5px solid #e0e0d8", padding:"10px 20px", display:"flex", alignItems:"center", gap:"12px" }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:"8px", position:'relative' }}>
          <div style={{ position:'relative' }}>
            ...
            {showNotif && <NotificationsPanel notifications={notifications} onClose={() => setShowNotif(false)} />}
          </div>
      ...
      <div style={{ display:"flex", flex:1, overflow:"hidden" }}>
        ...
        {d && (
          <div style={{ width:"320px", background:"#fff", borderLeft:"0.5px solid #e0e0d8", display:"flex", flexDirection:"column", overflow:"hidden" }}>
            ...
            <div style={{ margin:"12px 12px 0", borderRadius:"8px", overflow:"hidden", height:"120px", background:"linear-gradient(135deg,#d4c4b0,#b8a090)" }}>
            ...
            <div style={{ flex:1, overflowY:"auto", padding:"14px 16px" }}>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"12px", marginBottom:"12px" }}>
                <div><div style={{ fontSize:"10px", color:"#aaa" }}>Manufacturer</div><div style={{ fontSize:"13px", fontWeight:500, color:"#1a1a1a", marginTop:"2px" }}>{d.manufacturer}</div></div>
                <div><div style={{ fontSize:"10px", color:"#aaa" }}>Category</div><div style={{ fontSize:"13px", fontWeight:500, color:"#1a1a1a", marginTop:"2px" }}>{d.category}</div></div>
                <div><div style={{ fontSize:"10px", color:"#aaa" }}>Price / Unit</div><div style={{ fontSize:"13px", fontWeight:500, color:"#1a1a1a", marginTop:"2px" }}>{d.price}</div></div>
                <div><div style={{ fontSize:"10px", color:"#aaa" }}>Quantity</div><div style={{ fontSize:"13px", fontWeight:500, color:"#1a1a1a", marginTop:"2px" }}>{d.quantity}</div></div>
                <div><div style={{ fontSize:"10px", color:"#aaa" }}>Expiry Date</div><div style={{ fontSize:"13px", fontWeight:500, color:"#1a1a1a", marginTop:"2px" }}>{d.expiryDate}</div></div>
                <div><div style={{ fontSize:"10px", color:"#aaa" }}>Submitted</div><div style={{ fontSize:"13px", fontWeight:500, color:"#1a1a1a", marginTop:"2px" }}>{d.createdAt}</div></div>
              </div>
