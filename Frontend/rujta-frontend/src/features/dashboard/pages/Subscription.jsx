...
function PlanCard({ item, selectedId, onSelect }) {
  ...
  return (
    <div onClick={() => onSelect(item.id)}
      ...
      {isSelected && (
        <div className="absolute top-2.5 left-2.5 bg-[#3C623C] text-white text-[9px] px-2 py-0.5 rounded-full font-bold tracking-wide">✓ Selected</div>
      )}
      {item.ribbon && (
        <div className="absolute top-0 right-0 w-[80px] h-[80px] overflow-hidden rounded-tr-2xl pointer-events-none">
          <span className="absolute top-[18px] right-[-20px] bg-[#3C623C] text-white text-[9px] font-extrabold py-1 px-6 rotate-45 whitespace-nowrap shadow tracking-wide">{item.ribbon}</span>
        </div>
      )}
      ...
      {item.equiv && (
        <div className="text-[10px] text-[#9DC873] font-bold bg-[#f0f7e8] rounded-full px-3 py-1 inline-block mb-2 mx-auto">{item.equiv}</div>
      )}
      ...
      <div className="text-left space-y-1.5 mb-4">
        {item.features.map((f) => (
          <div key={f} className="flex items-center gap-2 text-[11px] text-gray-500">
            <MdCheckCircle size={13} className="text-[#9DC873] flex-shrink-0" />{f}
          </div>
        ))}
      </div>
      <button className={`w-full py-2.5 rounded-xl text-white text-xs font-bold transition-all ${isSelected ? "bg-[#3C623C]" : "bg-[#9DC873] hover:bg-[#3C623C]"}`}>
...
function SubscriptionPreview({ selectedPlan, status, subPlans }) {
  ...
  return (
    <div className="relative w-full overflow-hidden rounded-2xl shadow-2xl"
      style={{ minHeight: 380, background: plan ? "radial-gradient(circle at top left, #065f46, #3C623C)" : "radial-gradient(circle at top left, #374151, #1f2937)", fontFamily: "'DM Sans', sans-serif" }}>
      ...
      {plan?.ribbon && (
        <div style={{ position: "absolute", top: 0, left: 0, zIndex: 30, width: 130, height: 130, overflow: "hidden", pointerEvents: "none" }}>
          <div style={{ position: "absolute", top: 30, left: -36, width: 160, padding: "7px 0", background: "linear-gradient(135deg, #1a5c2a, #2d8c45)", transform: "rotate(-45deg)", textAlign: "center", boxShadow: "0 4px 14px rgba(0,0,0,0.3)" }}>
            <span style={{ fontSize: "0.65rem", fontWeight: 900, letterSpacing: "0.1em", textTransform: "uppercase", color: "#fff", whiteSpace: "nowrap" }}>{plan.ribbon}</span>
          </div>
        </div>
      )}
      <div className="relative z-10 w-full h-full flex flex-col justify-between px-8 py-8 gap-5">
        <div className="space-y-3">
          <div className="flex items-center gap-2 flex-wrap">
            <span className="text-[10px] font-bold px-3 py-1 rounded-full bg-white/20 text-white tracking-widest uppercase">{plan ? plan.label : "No Plan Selected"}</span>
            {hasStatus && (
              <span className={`text-[10px] font-bold px-3 py-1 rounded-full ${isActive ? "bg-green-500/30 text-green-200" : "bg-red-500/30 text-red-200"}`}>{status.status}</span>
            )}
          </div>
          ...
          {plan?.equiv && <div className="inline-block bg-white/10 rounded-full px-4 py-1 text-[11px] text-green-200 font-semibold">{plan.equiv}</div>}
        </div>
        {plan ? (
          <div className="space-y-2.5">
            {plan.features.map((f) => (
              <div key={f} className="flex items-center gap-2.5 text-sm text-white/85">
                <FiCheckCircle size={15} className="text-green-300 flex-shrink-0" />{f}
              </div>
            ))}
          </div>
        ) : (
        ...
        {hasStatus && (
          <div className="border-t border-white/15 pt-4 grid grid-cols-2 gap-3">
            {[
              ...
              { label: "Days Remaining", value: isActive ? `${status.daysRemaining} days` : "Expired", highlight: isActive ? "text-green-300" : "text-red-300" },
...
export default function Subscription() {
  const { user } = useAuth();
  const { loading: subLoading, error: subError, status, fetchStatus, create, renew } = useSubscription();
  const { paymentResult, loading: payLoading, error: payError, initiate, reset: resetPayment } = usePayment();
  ...
  useEffect(() => { if (paymentResult?.iframeUrl) setShowIframe(true); }, [paymentResult]);
  useEffect(() => { if (subError) toast.error(subError); }, [subError]);
  useEffect(() => { if (payError) toast.error(payError); }, [payError]);

  // ✅ subPlans built inside component from live pricing
  const subPlans = pricing ? [
    {
      id: "monthly", enumVal: 0, label: "MONTHLY",
      sublabel: "Simple monthly billing.",
      price: pricing.subscriptionMonthlyPrice.toLocaleString(),
      rawPrice: pricing.subscriptionMonthlyPrice,
      unit: "EGP / month",
      features: ["Cancel anytime", "Full platform access", "Standard support"],
    },
    {
      id: "yearly", enumVal: 1, label: "YEARLY",
      sublabel: "Best annual deal.",
      price: pricing.subscriptionYearlyPrice.toLocaleString(),
      rawPrice: pricing.subscriptionYearlyPrice,
      unit: "EGP / year",
      equiv: `Equiv. to ${Math.round(pricing.subscriptionYearlyPrice / 12).toLocaleString()} EGP/month`,
      ribbon: "SAVE 20%",
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      await initiate({
        ...
        amount:         selected?.rawPrice ?? 0, // ✅ uses live price
        currency:       "EGP",
        billingData: {
          ...
          shippingMethod: "PKG", postalCode: "11511",
    ...
    <div className="p-6 space-y-6 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-8 items-start">
        <div className="space-y-7">
            ...
            <section>
              ...
              <div className="bg-white rounded-2xl border border-[#e8f2e0] p-5">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className={`text-xs font-semibold px-3 py-1 rounded-full ${status.status === "Active" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-600"}`}>
                    {status.status}
                  </span>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  {[
                    ...
                    { label: "Days Remaining", value: status.status === "Active" ? `${status.daysRemaining} days` : "Expired", icon: MdTimelapse, className: status.status === "Active" ? "text-[#3C623C]" : "text-red-500" },
                ...
                {status.status === "Expired" && (
                  <div className="mt-3 flex items-center gap-2.5 rounded-xl bg-red-50 border border-red-100 px-3 py-2.5">
                    ...
                    <p className="text-xs text-red-600 font-medium">Your subscription has expired. Renew below to restore access.</p>
                  </div>
                )}
              </div>
            </section>
          ...
          <section>
            <SectionHeader icon={MdAutorenew} label={`${status ? "2" : "1"} · ${hasExisting ? "Renew / Change Plan" : "Choose a Plan"}`} />
            {subPlans.length === 0 ? (
              ...
              <div className="flex gap-4 items-stretch">
                {subPlans.map((item) => (
                  <PlanCard key={item.id} item={item} selectedId={selectedPlan} onSelect={setSelectedPlan} />
                ))}
              </div>
            )}
          </section>
        ...
        <div className="space-y-5 sticky top-6">
          ...
          <SubscriptionPreview selectedPlan={selectedPlan} status={status} subPlans={subPlans} />
          <div className="bg-white rounded-2xl border border-[#e0f0cc] shadow-sm px-5 py-4 flex justify-between items-center gap-4">
            <div className="min-w-0">
              ...
              <p className="text-base font-black text-[#3C623C] truncate">
                {selected ? `${selected.label} — ${selected.price} ${selected.unit}` : "Not selected yet"}
              </p>
            </div>
            <button
              disabled={!selectedPlan || isLoading || subPlans.length === 0}
              onClick={handleConfirm}
              className={`flex-shrink-0 px-6 py-3 rounded-xl text-white text-sm font-bold transition-all flex items-center gap-2
                ${selectedPlan && !isLoading ? "bg-[#3C623C] hover:bg-[#2d4a2d]" : "bg-[#c5ddb0] cursor-default"}`}>
          ...
          {!selectedPlan && <p className="text-xs text-gray-400">↑ Pick a plan to enable checkout.</p>}
        </div>
      </div>
    </div>
  );
