...
function normalizeResult(result) {
  ...
  if (Array.isArray(result.availableMedicines)) {
    return {
      ...
      unavailableMedicines: result.unavailableMedicines ?? result.notFoundMedicines ?? [],
    };
  }
  if (result.data && Array.isArray(result.data.availableMedicines)) {
    return {
      availableMedicines: result.data.availableMedicines,
      unavailableMedicines: result.data.unavailableMedicines ?? result.data.notFoundMedicines ?? [],
    };
  }
  if (Array.isArray(result.available)) {
    return { availableMedicines: result.available, unavailableMedicines: result.unavailable ?? [] };
  }
...
export default function ScanPrescription({ cart, setCart }) {
  ...
  useEffect(() => {
    if (unavailableMedicines.length > 0) setShowUnavailable(true);
  }, [result]);
  ...
  const handleUpload = async () => {
    ...
    await scan(images.map((img) => img.file));
  };
  ...
  const addToCart = (product) => {
    ...
    setTimeout(() => setAddedIds((prev) => ({ ...prev, [key]: false })), 2000);
  };
    ...
    <div className="flex min-h-screen flex-col items-center bg-gray-50 px-4 py-6 sm:px-6 lg:px-10">
      ...
      >
        ...
        <p className="mb-6 text-center text-sm text-gray-500 sm:mb-8 sm:text-base">
          Take photos or upload your prescription pages
        </p>

        {/* Upload buttons — always visible so user can keep adding */}
            ...
            {images.map((img, index) => (
              ...
              >
                ...
                <button
                  ...
                  className="absolute right-1 top-1 rounded-full bg-white p-1.5 shadow transition hover:bg-gray-100"
                >
                  <MdOutlineDelete className="text-base text-red-500" />
                </button>
                <span className="absolute bottom-1 left-1 rounded-lg bg-black/40 px-1.5 py-0.5 text-[10px] text-white">
                  {index + 1}
                </span>
              </motion.div>
            ))}
        ...
        >
          ...
          {loading
            ? "Processing..."
            : `Analyze ${images.length > 1 ? `${images.length} Prescriptions` : "Prescription"}`}
        </button>
                    ...
                    <div className="px-6 py-5">
                      <ul className="space-y-2.5">
                        {unavailableMedicines.map((med, idx) => (
                          <li key={idx} className="flex items-center gap-3 rounded-xl bg-gray-50 px-4 py-3 border border-gray-100">
            ...
            <>
              ...
              <div className="grid grid-cols-2 gap-4 sm:gap-6 md:grid-cols-3 lg:gap-8 xl:grid-cols-4">
                {availableMedicines.map((med, idx) => {
                  const medicine =
                    typeof med === "string"
                      ? { name: med, price: 0, description: "No description available", imageUrl: DEFAULT_IMAGE }
                  ...
                  return (
                    <motion.div
                      ...
                      style={{ borderRadius: 24, border: "1.5px solid #e8eee2", boxShadow: "0 1px 4px rgba(0,0,0,0.04)" }}
                      onMouseEnter={(e) => {
                    ...
                    >
                      ...
                      <div className="flex flex-1 flex-col p-4 sm:p-5">
                        <h3 className="font-semibold" style={{ color: "#3e6013", fontSize: 16, letterSpacing: "-0.1px" }}>
                          {medicine.name}
                        </h3>
                        ...
                        <div className="mt-5 flex items-center justify-between pt-4" style={{ borderTop: "1px solid #e8eee2" }}>
